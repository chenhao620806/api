...
from TestCase.TestBaoBiao.set_var_bao_biao import GetDataBaoBiao
...
# 读取Excel用例/需要运行的用例
excel_path = os.path.join(os.path.dirname(__file__), 'test_bao_biao.xlsx')
# # 测试环境
...
class TestHTTPRequest(StartEnd):
    ...
    def test_api(self, item):
        ...
        # 首先正则替换url/body ${}
        new_url = DoExcel().re_replaces(GetDataBaoBiao, item['路径'])
        new_data = DoExcel().re_replaces(GetDataBaoBiao, item['请求数据'])
        # 请求
        res = HttpRequest().http_request(new_url, eval(new_data), item['请求方法'], cookie=GetDataBaoBiao.Cookie)
        # 解析响应
        json_data = res.text
        # 消耗时间
        ...
        # cookies
        if res.cookies:
            setattr(GetDataBaoBiao, 'Cookie', res.cookies)

        # get通用
        if item['请求方法'] == 'get':
            VIEWSTATE = DoExcel.re_find(json_data, 'VIEWSTATE')
            setattr(GetDataBaoBiao, 'VIEWSTATE', VIEWSTATE)
            VIEWSTATEGENERATOR = DoExcel.re_find(json_data, 'VIEWSTATEGENERATOR')
            setattr(GetDataBaoBiao, 'VIEWSTATEGENERATOR', VIEWSTATEGENERATOR)
            EVENTVALIDATION = DoExcel.re_find(json_data, 'EVENTVALIDATION')
            setattr(GetDataBaoBiao, 'EVENTVALIDATION', EVENTVALIDATION)
        ...
        if item['title'] == '新建红升商品订单' and item['路径'] == '/jht/webapi/saleorder.aspx':
            try:
                ...
                if order_id:
                    setattr(GetDataBaoBiao, 'o_id', order_id)
