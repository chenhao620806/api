...
from TestCase.TestJht.set_var_jht import GetDataJht
...
# 读取Excel用例
excel_path = os.path.join(os.path.dirname(__file__), 'test_jht.xlsx')
# 测试环境
...
class TestHTTPRequest(StartEnd):
    ...
    def test_api(self, item):
        ...
        # 首先正则替换url/body ${}
        new_url = DoExcel().re_replaces(GetDataJht, item['路径'])
        new_data = DoExcel().re_replaces(GetDataJht, item['请求数据'])
        # 请求
        res = HttpRequest().http_request(base_url + new_url, eval(new_data), item['请求方法'], cookie=GetDataJht.Cookie)
        # 解析响应
        json_data = res.text
        # 消耗时间
        ...
        # cookies
        if res.cookies:
            setattr(GetDataJht, 'Cookie', res.cookies)
        ...
        if item['title'] == '新建红升商品订单' and item['路径'] == '/jht/webapi/saleorder.aspx':
            try:
                ...
                if order_id:
                    setattr(GetDataJht, 'o_id', order_id)
