...
from TestCase.TestKuaJing.set_var_kua_jing import GetDataKuaJing
...
# 读取Excel用例/需要运行的用例
excel_path = os.path.join(os.path.dirname(__file__), 'test_kua_jing.xlsx')
# 测试环境
...
class TestHTTPRequest(StartEnd):
    ...
    def test_api(self, item):
        ...
        # 首先正则替换url/body ${}
        new_url = DoExcel().re_replaces(GetDataKuaJing, item['路径'])
        new_data = DoExcel().re_replaces(GetDataKuaJing, item['请求数据'])
        # 请求
        res = HttpRequest().http_request(base_url + new_url, eval(new_data), item['请求方法'], cookie=GetDataKuaJing.Cookie)
        # 解析响应
        json_data = res.text
        # 消耗时间
        ...
        # cookies
        if res.cookies:
            setattr(GetDataKuaJing, 'Cookie', res.cookies)
        ...
        if item['title'] == '新建红升商品订单' and item['路径'] == '/jht/webapi/saleorder.aspx':
            try:
                ...
                if order_id:
                    setattr(GetDataKuaJing, 'o_id', order_id)
