...
from TestCase.TestGlobal.set_var_global import GetDataGlobal
...
# 读取Excel用例/需要运行的用例
excel_path = os.path.join(os.path.dirname(__file__), 'test_global.xlsx')
# 需要运行的用例
case = {
    'login': 'all',  # 必须先登录
}
...
class TestHTTPRequest(StartEnd):
    ...
    def test_api(self, item):
        ...
        new_url = DoExcel().re_replaces(GetDataGlobal, item['路径'])
        new_data = DoExcel().re_replaces(GetDataGlobal, item['请求数据'])
        # 请求
        res = HttpRequestJson().http_request(new_url, new_data, item['请求方法'], cookie=GetDataGlobal.Cookie)
        # 解析响应
        json_data = res.text
        # 消耗时间
        ...
        # cookies
        if res.cookies:
            setattr(GetDataGlobal, 'Cookie', res.cookies)
        ...
        if item['title'] == '获取创建的订单号码4':
            try:
                ...
                if o_id:
                    setattr(GetDataGlobal, 'o_id', o_id)
